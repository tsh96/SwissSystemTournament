...
function drawBoard() {
    content.innerHTML = "";
    chrome.runtime.sendMessage({type: MessageType.DRAWBOARD}, function (groups) {
...
function updateState(g, p, offset, state) {
    ...
    chrome.runtime.sendMessage({
        ...
        for (var p = 0; p < playersHTML_length; ++p) {
            var player = playersHTML[p];
...
chrome.runtime.sendMessage({type: MessageType.REQUESTBOARD}, function (_number_of_tables) {
    number_of_tables = parseInt(_number_of_tables);
    drawBoard();
    for (var n = 0; n < number_of_tables; ++n) {
        next();
    }
});
